# Ubicación: plugins/analysis/frequency/psd/psd_plugin_ui.py
...
class Ui_Psd(object):
    """
    UI para el plugin PSD (Actualizado con Modos de Cálculo).
    """
    def setupUi(self, Form):
        ...
        # ====== NUEVO: Calculation Mode ======
        self.lblModeTitle = QtWidgets.QLabel(self.panel)
        self.lblModeTitle.setFont(font_st)
        self.lblModeTitle.setText("Calculation Mode")
        self.vbox.addWidget(self.lblModeTitle)
        
        self.formMode = QtWidgets.QFormLayout()
        self.formMode.setLabelAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
        
        # Mode ComboBox
        self.modeLabel = QtWidgets.QLabel(self.panel)
        self.modeLabel.setText("Mode")
        self.modeComboBox = QtWidgets.QComboBox(self.panel)
        self.modeComboBox.addItems(["All Trials", "Average", "Individual"])
        self.formMode.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.modeLabel)
        self.formMode.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.modeComboBox)
        
        # Individual Trial Selector (inicialmente oculto)
        self.trialIndexLabel = QtWidgets.QLabel(self.panel)
        self.trialIndexLabel.setText("Trial Index")
        self.trialIndexSpinBox = QtWidgets.QSpinBox(self.panel)
        self.trialIndexSpinBox.setRange(0, 0)
        self.trialIndexSpinBox.setEnabled(False) # Deshabilitado hasta cargar trials
        self.formMode.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.trialIndexLabel)
        self.formMode.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.trialIndexSpinBox)

        # Ocultar selector de trial individual por defecto
        self.trialIndexLabel.setVisible(False)
        self.trialIndexSpinBox.setVisible(False)
        
        self.vbox.addLayout(self.formMode)
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_("PSD", "PSD"))
        self.pushButton.setText(_("PSD", "Calculate PSD"))